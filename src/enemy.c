...
static int g_playerId = -1;
static int g_worldHeat = 0;

Entity g_entities_storage[MAX_ENTITIES];
int g_entityCount_storage = 0;
...
void EnemyAI(Entity* e, float dt) {
    ...
    Entity* player = NULL;
    for (int i = 0; i < g_entityCount; i++) {
        if (g_entities[i].active && g_entities[i].type == ENTITY_PLAYER) {
            player = &g_entities[i];
            break;
        }
    }
    ...
    switch (e->faction) {
        ...
        case FACTION_JERUSALEM:
            GangAI(e, player, dist, dt);
            break;
        case FACTION_POLICE:
            PoliceAI(e, player, dist, dt);
            break;
    }
}
...
void GangAI(Entity* e, Entity* player, float dist, float dt) {
    if (dist < 400.0f) {
        int target_id = EntityFindNearestTarget(e, FACTION_PLAYER, 400.0f);
        if (target_id >= 0) {
            Entity* target = &g_entities[target_id];
            e->currentTargetId = target_id;
            ...
            if (dist < 80.0f && e->weapon != WEAPON_NONE) {
                if (e->weapon == WEAPON_KNIFE || e->weapon == WEAPON_FISTS) {
                    ...
                    Entity* bullet = EntityCreate(e->faction, e->x, e->y);
                    if (bullet) {
...
void PoliceAI(Entity* e, Entity* player, float dist, float dt) {
    int heat = g_worldHeat;

    if (heat > 50 || dist < 300.0f) {
        int target_id = EntityFindNearestTarget(e, FACTION_MUNGIKI, 500.0f);
        if (target_id < 0) target_id = EntityFindNearestTarget(e, FACTION_TALIBAN, 500.0f);
        if (target_id < 0) target_id = EntityFindNearestTarget(e, FACTION_JERUSALEM, 500.0f);
        if (target_id < 0 && player->heat > 0) target_id = EntityFindNearestTarget(e, FACTION_PLAYER, 500.0f);
...
void EnemySpawnWave(int wave) {
    ...
    for (int i = 0; i < count; i++) {
        ...
        Entity* e = EntityCreate(faction, x, y);
        if (e) {
            e->health = 50 + wave * 10;
    ...
    if (wave % 5 == 0) {
        ...
        Entity* boss = EntityCreate(FACTION_MUNGIKI, x, y);
        if (boss) {
...
bool WorldIsWalkable(float x, float y) {
    ...
    int local_x = (int)x % ROAD_GRID_SIZE;
    int local_y = (int)y % ROAD_GRID_SIZE;
...
Entity* EntityCreate(Faction faction, float x, float y) {
    if (g_entityCount >= MAX_ENTITIES) return NULL;

    Entity* e = &g_entities[g_entityCount++];
